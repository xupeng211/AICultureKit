...
class TestQualityTools:
    ...
    @patch('subprocess.run')
    def test_run_quality_check_with_failures(self, mock_run):
        """测试质量检查失败情况"""
    ...
    def test_setup_python_linting(self):
        """测试Python linting设置"""
        tools = QualityTools(str(self.project_path))
...
class TestProjectTemplate:
    ...
    def test_create_python_project(self, mock_repo_class):
        """测试创建Python项目"""
        # Mock git repository
        mock_repo = MagicMock()
    ...
    def test_create_js_structure(self):
        """测试JavaScript项目结构创建"""
        template = ProjectTemplate()
        project_path = self.target_path / self.project_name
        project_path.mkdir(parents=True)
    ...
    def test_get_ci_workflow_content(self):
        """测试CI工作流内容生成"""
        template = ProjectTemplate()
    ...
    def test_get_cd_workflow_content(self):
        """测试CD工作流内容生成"""
        template = ProjectTemplate()
...
def test_integration_create_and_check_project(temp_project_dir):
    """集成测试：创建项目并检查质量"""
