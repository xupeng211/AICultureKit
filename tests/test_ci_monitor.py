...
class TestGitHubCIMonitor:
    ...
    def setup_method(self):
        ...
        # 创建模拟的Git仓库结构
        (self.test_project_root / ".git").mkdir()
        
        with patch.dict('os.environ', {'GITHUB_TOKEN': 'test_token_123'}):
            self.monitor = GitHubCIMonitor(str(self.test_project_root))
    ...
    def test_failure_analysis_dependency_conflict(self):
        ...
        assert any("requirements.txt" in suggestion for suggestion in analysis["suggestions"])
    ...
    def test_failure_analysis_missing_dependency(self):
        ...
        assert analysis["severity"] == "high"
        assert any("requirements" in suggestion for suggestion in analysis["suggestions"])
    ...
    def test_api_error_handling(self, mock_get):
        ...
        # 模拟认证错误
        mock_response = Mock()
        mock_response.raise_for_status.side_effect = Exception("认证失败")
        mock_get.return_value = mock_response
    ...
    def test_monitor_without_repo_info(self):
        ...
        with patch('subprocess.run') as mock_run:
            mock_run.return_value.returncode = 1  # 模拟非Git仓库
...
class TestCIMonitorIntegration:
    ...
    @patch('subprocess.run')
    def test_git_repository_detection(self, mock_run):
