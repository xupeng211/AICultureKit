...
class TestAssertionValidation:
    ...
    '''
                    for i, assertion in enumerate(failed_assertions)
                    if assertion
                ]
            )
        }
...
'''
    ...
    def _generate_generic_tests(
        self, category: str, issues: List[Dict]
    ) -> Optional[str]:
        """生成通用测试"""
            ...
            config["repos"].append(
                {
                    ...
                    "rev": "1.7.5",
                    "hooks": [
                        {"id": "bandit", "args": ["-r", "src/"], "exclude": "tests/"}
                    ],
                }
            )
    ...
    def enhance_ci_workflow(self, issues: List[Dict]) -> bool:
        ...
        if "ruff" in tools_with_issues:
            steps.extend(
                [
                    ...
                    {"name": "Ruff lint check", "run": "ruff check src/ tests/"},
                ]
            )

        if "mypy" in tools_with_issues:
            steps.append({"name": "MyPy type check", "run": "mypy src/"})

        if "bandit" in tools_with_issues:
            steps.append({"name": "Security scan", "run": "bandit -r src/"})
        ...
        try:
            ...
            with open(workflow_path, "w", encoding="utf-8") as f:
                yaml.dump(
                    enhanced_workflow, f, default_flow_style=False, sort_keys=False
                )
...
class DefenseGenerator:
    ...
    def generate_comprehensive_defenses(
