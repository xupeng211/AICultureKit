...
def analyze_coverage_gaps(coverage_file: Path = Path("coverage.json")) -> List[Dict]:
    ...
    files_analysis = []
    omit_patterns = ["tests/", "__main__.py", "migrations/", "experimental/", "venv/", "build/", "dist/"]
    ...
    for file_path, file_data in data["files"].items():
        ...
        if total_lines > 0:  # 只分析有代码的文件
            files_analysis.append({
                ...
                "priority_score": missing_lines * (1 - coverage_percent / 100)  # 缺失行数 × 覆盖率不足比例
            })
...
def print_coverage_analysis(top_n: int = 10):
    ...
    avg_coverage = sum(f["coverage_percent"] for f in files) / len(files) if files else 0
