...
class TestValidator:
    ...
    def validate_test_coverage(self, test_files: List[str]) -> Dict[str, Any]:
        ...
        try:
            # 运行覆盖率测试
            result = subprocess.run(
                ['pytest'] + [f'tests/{tf}' for tf in test_files] + 
...
class LintValidator:
    ...
    def validate_lint_configs(self, config_files: List[str]) -> Dict[str, Any]:
        ...
        results['success_rate'] = (results['valid_configs'] / max(results['total_configs'], 1)) * 100
...
class CIWorkflowValidator:
    ...
    def validate_ci_workflows(self, workflow_files: List[str]) -> Dict[str, Any]:
        ...
        results['success_rate'] = (results['valid_workflows'] / max(results['total_workflows'], 1)) * 100
    ...
    def _validate_single_workflow(self, workflow_path: Path) -> Dict[str, Any]:
        ...
        if not workflow_path.exists():
            return {'status': 'not_found', 'message': f'工作流文件不存在: {workflow_path}'}
        ...
        try:
            ...
            for job in jobs.values():
                for step in job.get('steps', []):
                    ...
                    if any(keyword in step_name for keyword in ['defense', 'validation', 'guardian']):
...
class EffectivenessValidator:
    ...
    def validate_defense_effectiveness(self, original_issues: List[Dict], defenses: Dict[str, List[str]]) -> Dict[str, Any]:
        """验证防御机制的有效性"""
        results = {
...
'''
    ...
    def _analyze_issue_coverage(self, issues: List[Dict], defenses: Dict[str, List[str]]) -> Dict[str, Any]:
        """分析问题覆盖率"""
...
class DefenseValidator:
    ...
    def validate_all_defenses(self, defenses: Dict[str, List[str]], original_issues: List[Dict] = None) -> Dict[str, Any]:
        """验证所有防御机制"""
        start_time = time.time()
        ...
        if defenses.get('pre_commit_hooks'):
            ...
            precommit_results = self.precommit_validator.validate_precommit_hooks(defenses['pre_commit_hooks'])
            validation_results['detailed_results']['pre_commit'] = precommit_results
        
        # 4. 验证CI工作流
        if defenses.get('ci_workflows'):
            click.echo("  ⚙️ 验证CI工作流...")
            workflow_results = self.workflow_validator.validate_ci_workflows(defenses['ci_workflows'])
        ...
        # 计算总体评分
        validation_results['overall_score'] = self._calculate_overall_score(validation_results['detailed_results'])
    ...
    def _generate_validation_summary(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'effectiveness' in detailed:
            ...
            summary['component_status']['effectiveness'] = {
                ...
                'coverage_rate': effectiveness_result.get('issue_coverage', {}).get('coverage_rate', 0),
                'reproducible_issues': effectiveness_result.get('test_reproduction', {}).get('reproducible_issues', 0)
            }
...
def main(defenses_file, issues_file, project_root, output, tests_only, 
         configs_only, effectiveness_only, summary, detailed):
    """
    🔍 防御机制验证系统
    ...
    if tests_only:
        ...
        lint_results = validator.lint_validator.validate_lint_configs(defenses.get('lint_configs', []))
        click.echo(f"📊 配置验证结果: {lint_results.get('success_rate', 0):.1f}% 有效率")
    elif effectiveness_only:
        if original_issues:
            effectiveness_results = validator.effectiveness_validator.validate_defense_effectiveness(
                original_issues, defenses
            )
            click.echo(f"📊 防御效果评分: {effectiveness_results.get('effectiveness_score', 0):.1f}")
        else:
