...
class GitHubActionsUpdater:
    ...
    def _add_defense_job(self, workflow: Dict, workflow_path: Path, defenses: Dict[str, List[str]]) -> bool:
        """添加新的防御作业"""
        defense_job = {
...
class RequirementsUpdater:
    ...
    def update_dev_requirements(self, defenses: Dict[str, List[str]]) -> bool:
        ...
        try:
            ...
            # 如果有安全检查，确保bandit包存在
            if any('security' in str(files) for files in defenses.values()) and 'bandit' not in content:
...
def main(defenses_file, project_root, validate_only, makefile_only, 
         github_only, docs_only, summary):
    """
    🔧 自动CI配置更新器
