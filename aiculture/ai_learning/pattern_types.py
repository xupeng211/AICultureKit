...
class PatternAnalyzer:
    ...
    def _extract_examples(self, data: List[str], pattern_value: str, max_examples: int = 5) -> List[str]:
        """提取模式示例"""
        examples = []
        for item in data:
        ...
        if snake_case_count > camel_case_count and snake_case_count > pascal_case_count:
            ...
            if confidence >= self.confidence_threshold:
                patterns.append(ProjectPattern(
                    ...
                    examples=self._extract_examples([n for n in names if self._is_camel_case(n)], "")
                ))
            ...
            if confidence >= self.confidence_threshold:
                patterns.append(ProjectPattern(
                    ...
                    examples=self._extract_examples([n for n in names if self._is_pascal_case(n)], "")
                ))
