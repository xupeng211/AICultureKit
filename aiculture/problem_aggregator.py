...
class ProblemAggregator:
    ...
    def display_problem_summary(self, problems: Dict[str, Any]) -> None:
        ...
        for priority in problems['fix_priority']:
            ...
            print(f"   {priority['priority']}. {priority['category']} ({priority['count']} 个) - {blocking_text}")
        ...
        if categories['culture_warnings']:
            other_warnings = [w for w in categories['culture_warnings'] 
        ...
        print(f"   2. 按优先级顺序逐一解决问题")
        print(f"   3. 每个问题都有具体的修复建议")
        print(f"   4. 修复完成后重新运行检查验证")
        print(f"   5. 所有错误解决后即可成功推送")
