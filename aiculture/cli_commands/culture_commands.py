...
@click.option('--type', '-t', multiple=True,
              help='检查类型 (accessibility, i18n, performance)')
...
@click.option('--path', '-p', default='.', 
              help='项目路径')
@click.option('--output', '-o', default='culture-report.md',
              help='报告输出文件')
...
@click.option('--path', '-p', default='.', 
              help='项目路径')
@click.option('--strictness', '-s', default=0.7, type=float,
              help='执行严格度 (0.0-1.0)')
    ...
    try:
        ...
        if result.patterns:
            ...
            for pattern in result.patterns[:5]:  # 只显示前5个
                click.echo(f"  • {pattern.pattern_name}: {pattern.pattern_value} (置信度: {pattern.confidence:.2f})")
