...
from .exceptions import AICultureError, get_error_severity, is_retryable_error
...
class ErrorHandler:
    ...
    def _execute_fallback(
        ...
        try:
            ...
            self.logger.error(
                "降级处理也失败了", error=fallback_error, original_error=original_error
            )
            raise original_error
...
def handle_errors(
    ...
    try:
        ...
        severity = get_error_severity(error)
        logger.error(f"操作 {operation_name} 发生错误", error=error, severity=severity)
...
retry_with_fallback = with_error_handling(
    RetryConfig(max_attempts=2), FallbackConfig(fallback_value=None)
)
