...
def process_file(file_path: str) -> dict:
    ...
    if not path.exists():
        raise ResourceError("文件不存在", resource_type="file", resource_path=str(path))
    ...
    if random.random() > 0.8:
        raise ProcessingError(
            "文件处理失败", operation="file_processing", file_path=str(path)
        )
...
def demo_basic_logging():
    ...
    # 设置上下文
    logger.set_context(user_id="user123", request_id="req456", component="demo")
            ...
            for file_path in files:
                try:
                    ...
                    error_monitor.record_error(
                        e, operation="file_processing", context={"file": file_path}
                    )
    except Exception as e:
    ...
    for op_name, duration in operations:
        ...
        with logger.operation_context(op_name):
            ...
            performance_tracker.record_operation(
                operation=op_name, duration_ms=actual_duration, status="success"
            )
