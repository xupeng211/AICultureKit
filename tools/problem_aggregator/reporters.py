    ...
    def generate_report(self, result: Dict[str, Any]) -> str:
        ...
        if blocking_count > 0:
            ...
            for i, problem in enumerate(blocking_problems, 1):
                lines.append(f"{i}. **{problem.get('file', 'N/A')}:{problem.get('line', 0)}** - {problem.get('message', '')}")
        ...
        lines.append("python -m tools.problem_aggregator.aggregator --md artifacts/problems_report.md")
        lines.append("```")
        lines.append("")
...
class DashboardReporter:
    ...
    def generate_dashboard(self, result: Dict[str, Any]) -> str:
        ...
        if file_stats:
            lines.append("🔥 风险热点:")
            for file_path, risk_score in sorted(file_stats.items(), key=lambda x: x[1], reverse=True)[:5]:
