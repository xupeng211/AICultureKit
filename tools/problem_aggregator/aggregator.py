...
class ProblemAggregator:
    ...
    def aggregate_problems(
        ...
        print("  📝 运行代码风格检查...")
        try:
            lint_problems = self.lint_adapter.run_ruff(files or [])
        ...
        try:
            security_problems = []
            bandit_problems = self.security_adapter.run_bandit(files or [])
            secrets_problems = self.security_adapter.run_detect_secrets(files or [])
